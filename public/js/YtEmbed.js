...
var FixedPlayingListKind='channel';//現在FixedVideo 撥放的是 channel||personal
...
function FixedPlayerBuildYoutubeVideo(ViID,KIND){//建立 Yt Fixed PlayerElement    
    ...
    if(FixedYtElem==null){//Fixed VideoPlayer Not Exisit        
        ...
        $('#YtInfinityPlayer').append(`
            ...
            <button class='fivideobtn' onclick="FixedVideoListToggle()">PlayList</button>
            <button id='FixedPauseBtn' class='fivideobtn' onclick="FixedVideoTogglePause()">暫停</button>
            <button class='fivideobtn' onclick="FixedPlayPrevVideo()">Prev</button>
            <button class='fivideobtn' onclick="FixedPlayNextVideo()">Next</button>
            <button class='fivideobtn' onclick="CloseFixedVideo()">關閉</button>        
            </p>
...
function InitRenderFixedVideoList(){//初始化Render Fixed List    
    ...
    if(FixedListShow==false)FixedListClass='';
    $('#YtInfinityPlayer').append(`
    <div id='FixedVideoList' class='${FixedListClass}'>
        ...
        <div class='AddLinkPanel'>
            <div class='tops'>
                ...
                <button>增加Video網址</button>
            </div>
            <div class='bottoms'>
                <div class='show'>
                    <input type="text" class="Lists" placeholder="請輸入歌單簡介">
                    <button id="FixedSendShareList">分享</button>
                </div>
                <div>
                    <h1>自訂歌單增加Youtube網址</h1>
                    <input type="text" class="links" placeholder="請輸入Youtube網址">
                    <input type="text" class="title" placeholder="請輸入影片標題">
...
$(document).on("click",'button#ShareListBtnAttr',function(){//在 聊天室中 使用/取消 分享List
    ...
    if($(this).hasClass('useList')){
        $('#ShareContentArea>p>input').attr('placeholder','請輸入List名稱');
        $(this).text('取消分享');
    }
    else{
